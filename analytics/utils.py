...
def calculate_analytics():
    ...
    for place in places:
        ...
        total_purchases = Check.objects.filter(place_of_purchase=place.place_name).count()

        # Расситываем средний чек в данном месте
        average_receipt = Check.objects.filter(
            place_of_purchase=place.place_name).aggregate(average_receipt=Avg('total_amount'))
        ...
        for category in categories:
            total_spent = Check.objects.filter(
                place_of_purchase=place.place_name, category=category).aggregate(total_spent=Sum('total_amount'))
            average_receipt_category = Check.objects.filter(
                place_of_purchase=place.place_name, category=category).aggregate(average_receipt=Avg('total_amount'))
